...
function UploadModal({
  ...
  const submit = async (files: File[]) => {
    ...
    try {
      if (purpose === "planned") {
        ...
        const s = await importPlannedRoute(gpx);
        onImportedRoute(s.id);
        return;
      }
  ...
  return (
    <FocusLock open onClose={onClose} labelledBy="upload-title" variant="modal">
      ...
      {purpose === "planned" ? (
        ...
        <>
          ...
          <div className="kind-toggle kind-toggle--secondary" role="group" aria-label="Ride type">
            ...
            <button type="button" className={kind === "race" ? "active" : ""} onClick={() => setKind("race")}>
              Ultra race
            </button>
          </div>
        </>
      ...
      >
        ...
        <div className="dropzone__title">{busy ? "Importing…" : "Drop files here, or click to choose"}</div>
